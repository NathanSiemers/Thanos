    ...
    observe({
      ...
      lapply(vars, function(col) {
        observeEvent(input[[paste0("reset_", col)]], {
          ...
          # Force reactivity for filtered_data and dependent outputs
          session$sendCustomMessage("force_reactivity", list(var = col))
        }, ignoreInit = TRUE)
      })
    })

    # Custom message to force reactivity
    session$onFlushed(function() {
      session$sendCustomMessage("force_reactivity", list())
    })
