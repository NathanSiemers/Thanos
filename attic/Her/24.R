...
thanos_allnone <- function(input, output, session, data_list, x) {
  ...
  if (as.numeric(act) %% 2 == 0) {
    updateCheckboxGroupInput(session, paste0("filter_", x), label = paste("Filter", x), choices = allchoices, selected = allchoices, inline = TRUE)
  } else {
    updateCheckboxGroupInput(session, paste0("filter_", x), label = paste("Filter", x), choices = allchoices, selected = character(0), inline = TRUE)
  }
}
      ...
      lapply(vars, function(col) {
        observeEvent(input[[paste0("action_", col)]], {
          if (is.factor(df[[col]]) || is.character(df[[col]])) {
            thanos_allnone(input, output, session, df, col)
          }
        }, ignoreInit = TRUE)
