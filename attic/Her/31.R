# Module File: dynamic_filter_module.R
...
DynamicFilterModuleUI <- function(id) {
  ...
  tagList(
    ...
    conditionalPanel(
      ...
      downloadButton(ns("download_data"), "Download Filtered Data") # Download button
    )
  )
}

DynamicFilterModuleServer <- function(id, data, plot_width = 800, plot_height = 200, defaultFilter = NULL, showTable = TRUE, download = TRUE) {
  moduleServer(id, function(input, output, session) {
        ...
        tagList(
          if (is.numeric(df[[col]])) {
            tagList(
              ...
              checkboxInput(ns(paste0("na_", col)), label = "Include NAs", value = FALSE)
            )
    ...
    filtered_data <- reactive({
      ...
      for (col in vars) {
        ...
        include_na_flag <- ifelse(is.null(include_na[[col]]), FALSE, include_na[[col]])
    ...
    # Provide a download handler for filtered data
    if (download) {
