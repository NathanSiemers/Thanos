...
thanos_allnone <- function(input, output, session, data_list, x) {
  ...
  if (as.numeric(act) %% 2 == 0) {
    updateCheckboxGroupInput(session, paste0("filter_", x), x, choices = allchoices, selected = allchoices, inline = TRUE)
  } else {
    updateCheckboxGroupInput(session, paste0("filter_", x), x, choices = allchoices, selected = character(0), inline = TRUE)
  }
}
