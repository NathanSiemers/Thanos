...
thanosModuleServer <- function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    # Includes .init logic if no filters exist
    plot_it <- function(data_list, selected, x) {
      ...
      if (length(filter_most) == 0) {
        # If there are no "other" filters, nothing has filtered out so far
        reduced <- rep(TRUE, length(var))
      } else {
    ...
    combined_filter <- reactive({
      vecs <- selected_list()
      if (length(vecs) == 0) {
        # If no columns selected, keep all rows from the first data frame
        rep(TRUE, nrow(data_list[[1]]))
      } else {
        reduce(vecs, `&`)
      }
    })
...
server <- function(input, output, session) {
  ...
  output$data <- renderTable({
    ...
    # If final_filter() is all TRUE or a subset
